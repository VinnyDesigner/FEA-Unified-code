-- =====================================================================
-- FEA Unified — MWQ domain database  (fea_mwq)
-- Marine Water Quality domain tables + per-module operational tables
-- (otps, reports, audit_logs). Identity/access lives in fea_higher_level.
--
-- These operational tables reference a user by a plain integer "UserID"
-- with NO cross-database foreign key (PostgreSQL cannot enforce FKs across
-- databases; integrity is enforced in application code).
--
-- AUTHORITATIVE SCHEMA = Prisma (server/prisma/mwq/schema). Generated DDL,
