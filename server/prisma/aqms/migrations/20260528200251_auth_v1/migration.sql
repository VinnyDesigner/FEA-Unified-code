-- Migration: auth_v1 (AQMS DB)
-- Runs after auth_v1_enum which committed MWQ_MEMBER and AQMS_MEMBER into the Role enum.
-- Migrates VIEWER rows -> AQMS_MEMBER, adds accountStatus index, creates refresh_tokens table.
--
...
-- Migrate existing VIEWER rows to AQMS_MEMBER
UPDATE "users" SET "role" = 'AQMS_MEMBER' WHERE "role" = 'VIEWER';
