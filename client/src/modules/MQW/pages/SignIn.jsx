...
const SignIn = () => {
  ...
  return (
    ...
    >
      <form className="w-full flex flex-col gap-3 sm:gap-5" onSubmit={(e) => { e.preventDefault(); navigate('/MQW/dashboard'); }}>
        <FormInput 
          label={t('auth.email')} 
          type="email" 
        ...
        <div className="flex items-center justify-between mt-[-4px]">
          ...
          <Link to="/MQW/forgot-password" className="text-xs font-bold text-white hover:underline hover:text-[#19D9F3] transition-colors tracking-wide underline underline-offset-2">
        ...
        <p className="text-center text-[11px] sm:text-xs text-white font-medium tracking-wide mt-1 sm:mt-2">
          {t('auth.noAccount')} <Link to="/MQW/signup" className="text-[#19D9F3] font-bold tracking-wide hover:underline">{t('auth.signUp')}</Link>
        </p>
      </form>
    </AuthCard>
  );
