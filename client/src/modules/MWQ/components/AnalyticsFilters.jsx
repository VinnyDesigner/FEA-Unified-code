...
const AnalyticsFilters = ({
  ...
  const getBuoyTriggerLabel = () => {
    if (isBuoysAnalytics && Array.isArray(selectedBuoy)) {
      ...
      if (selectedBuoy.length === stations.length && stations.length > 0) return t("stations.allStations", "All Stations");
      if (selectedBuoy.length === 1) return selectedBuoy[0]?.buoyName || '';
      return isRtl ? `${selectedBuoy.length} محطات` : `${selectedBuoy.length} Stations`;
    }
    return selectedBuoy?.buoyName || t("analytics.selectStation", "Select Station...");
  };
              ...
              <div className="flex flex-col gap-4">
                {isBuoysAnalytics && (
                  <button
                    ...
                    onClick={() => {
                      if (setSelectedBuoy) {
                        if (Array.isArray(selectedBuoy) && selectedBuoy.length === stations.length && stations.length > 0) {
                          setSelectedBuoy([]); // Deselect all
                        } else {
                          setSelectedBuoy([...stations]); // Select all
                        }
                      }
                    }}
                  >
                    <div
                      className={`w-[17px] h-[17px] rounded-[4px] border-2 transition-all flex items-center justify-center ${
                        Array.isArray(selectedBuoy) && stations.length > 0 && selectedBuoy.length === stations.length ? 'border-white bg-white' : 'border-white/40 bg-transparent group-hover:border-white/60'
                      }`}
                    >
                      {Array.isArray(selectedBuoy) && stations.length > 0 && selectedBuoy.length === stations.length && (
                ...
                {stations.map((station) => {
                  const isChecked = isBuoysAnalytics && Array.isArray(selectedBuoy)
                    ? selectedBuoy.some(b => b.id === station.id)
                    : selectedBuoy?.id === station.id;
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (isBuoysAnalytics && Array.isArray(selectedBuoy)) {
                          const newSelection = selectedBuoy.some(b => b.id === station.id)
                            ? selectedBuoy.filter(b => b.id !== station.id)
                    ...
                    >
                      ...
                      <span className="text-white text-[13px] font-semibold">{station.buoyName}</span>
                    </button>
                  );
                })}
              </div>
