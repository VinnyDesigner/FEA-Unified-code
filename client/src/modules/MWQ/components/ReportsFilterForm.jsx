...
const ReportsFilterForm = ({ isDesktop = false, onApply, stations: stationsProp = [], parameters: parametersProp = [] }) => {
  ...
  // stations and parameters come from props (loaded by parent from API).
  // Fall back to empty arrays if not yet loaded.
  const stations = stationsProp.length > 0 ? stationsProp : [];
  const monitoringTypes = ['Sonde Information', 'Weather Information'];
  const parameters = parametersProp.length > 0 ? parametersProp : [];
              ...
              <span className="truncate">
                {formData.station.length === 0
                  ...
                  : formData.station.length === 1
                    ? (() => { const s = stationsProp.find(b => b.id === formData.station[0]); return s ? getTransLabel(s.buoyName) : formData.station[0]; })()
              ...
              >
                {stations.map((station) => (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleSelection('station', station.id);
                    }}
                  >
                    <div className={`w-4 h-4 rounded flex items-center justify-center transition-colors ${formData.station.includes(station.id) ? 'bg-[#009FAC]' : 'border border-white/40'}`}>
                      {formData.station.includes(station.id) && <svg viewBox="0 0 14 14" fill="none" className="w-3 h-3 text-white"><path d="M3 7.5L5.5 10L11 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </div>
                    {getTransLabel(station.buoyName)}
                  </button>
                ))}
              </div>,
              document.body
              ...
              <span className="truncate">
                {formData.parameter.length === 0
                  ...
                  : formData.parameter.length === 1
                    ? (() => { const p = parametersProp.find(x => x.id === formData.parameter[0]); return p ? getTransLabel(p.name) : formData.parameter[0]; })()
              ...
              >
                {parameters.map((param) => (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleSelection('parameter', param.id);
                    }}
                  >
                    <div className={`w-4 h-4 rounded shrink-0 flex items-center justify-center transition-colors ${formData.parameter.includes(param.id) ? 'bg-[#009FAC]' : 'border border-white/40'}`}>
                      {formData.parameter.includes(param.id) && <svg viewBox="0 0 14 14" fill="none" className="w-3 h-3 text-white"><path d="M3 7.5L5.5 10L11 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </div>
                    {getTransLabel(param.name)}
                  </button>
                ))}
              </div>,
              document.body
