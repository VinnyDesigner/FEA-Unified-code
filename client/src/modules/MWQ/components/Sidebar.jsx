...
import { logout as apiLogout } from '../../../lib/queries';

const navItems = [
  { icon: '/assets/nav/home.png', labelKey: 'nav.dashboard', path: '/MWQ/dashboard' },
  { icon: '/assets/nav/analytics.png', labelKey: 'nav.misAnalytics', path: '/MWQ/mis-analytics' },
  { icon: '/assets/nav/reports.png', labelKey: 'nav.reports', path: '/MWQ/reports' },
  { icon: '/assets/nav/help.png', labelKey: 'nav.faq', path: '/MWQ/faq' },
];
...
const Sidebar = () => {
  ...
  // Invalidate the server session (refresh token) before clearing local state.
  const handleLogout = async () => {
    const { module, refreshToken, logout } = useAuthStore.getState();
    try { if (module && refreshToken) await apiLogout(module, refreshToken); } catch { /* ignore */ }
    logout();
    navigate('/MWQ/signin');
  };
