...
const AnalyticsFilters = () => {
  ...
  const updateFilterPos = useCallback(() => {
    if (filterBtnRef.current) {
      ...
      setFilterPos({
        ...
        left: rect.right + window.scrollX - 420 // Align to right of button (approx width 420)
      });
    }
  }, []);
  ...
  return (
    <div className="flex items-center gap-3">
      {/* Date Picker 1 */}
      <div 
        className="flex items-center gap-2 px-4 py-2 text-xs transition-all cursor-pointer"
        style={{
          borderRadius: '24px',
      ...
      <button 
        ...
        className="flex items-center gap-2 px-6 py-2 text-xs font-bold transition-all hover:brightness-110 active:scale-95"
        style={{
          borderRadius: '24px',
          border: '1px solid rgba(255, 255, 255, 0.30)',
          background: 'radial-gradient(50% 50% at 50% 50%, rgba(255, 255, 255, 0.20) 0%, rgba(255, 255, 255, 0.25) 100%)',
      ...
      {isFilterOpen && createPortal(
        <div 
          ref={filterMenuRef}
          className="fixed z-[9999] p-8 flex flex-col gap-6 shadow-2xl"
          style={{
            top: filterPos.top,
            left: filterPos.left,
            width: '420px',
